...
export function StatistichePersona({
  ...
  const adesioni = righe.filter((r) => r.status === 'PRESENTE').length;
  const presenze = svolte.filter((r) => r.presente === true).length;
   ...
   */
  const attese = svolte.filter((r) => r.status === 'PRESENTE' && r.presente !== null).length;
  const mancate = svolte.filter((r) => r.status === 'PRESENTE' && r.presente === false).length;
  ...
  return (
    <div className="mb-6 grid grid-cols-2 gap-3 lg:grid-cols-4">
      <Statistica
        etichetta="Presenze"
        valore={presenze}
        dettaglio={`su ${svolte.length} ${svolte.length === 1 ? 'attività svolta' : 'attività svolte'}`}
